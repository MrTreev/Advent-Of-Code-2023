...
bool validate_game(const bool is_valid) {
    ...
    // clang-format off
    logger.debug(std::string("Valid: ") + (local_valid ? "YES " : "NO  ") +
              ...
              "\tMax: R" + std::to_string(r_max) + " G" + std::to_string(g_max) + " B" + std::to_string(b_max));
    // clang-format on
...
std::size_t valid_game_no(std::string &line) {
    ...
    if (g_token == std::string::npos) { logger.fatal("No ':' found\nIn line: " + line); }
    this_no = 0;
    r_no = 0, g_no = 0, b_no = 0;
    bool is_valid = true;
    for (std::string::iterator iter = line.begin() + g_token; iter != line.end(); iter++) {
        switch (*iter) {
            ...
            case ' ':
                ...
                break;
            case aoc_numeric: break;
            case aoc_alpha: break;
            default: logger.fatal("Unrecognised Character: '" + std::to_string(*iter) + "'\nIn line: " + line);
        }
    }
...
int main() {
    ...
    for (std::string line; std::getline(std::cin, line);) {
        ...
        total                          += this_game_no;
        power                          += this_power;
        logger.info(std::string("Valid: ") + ((this_game_no != 0) ? "YES" : "NO ") +
                    " Power: " + std::to_string(this_power) + "\tTotal: " + std::to_string(total) + line);
